...
export const expectDifferentSetCookieSessionId = (
  ...
  expect(cookieValue).toMatch(new RegExp(`${sessionIdString}=(?!${sessionId}).*; Path=/; HttpOnly; SameSite=Strict`));
};
...
export const expectSetCookieSessionId = (
  ...
  expect(cookieValue).toEqual(`${sessionIdString}=${sessionId}; Path=/; HttpOnly; SameSite=Strict`);
};
