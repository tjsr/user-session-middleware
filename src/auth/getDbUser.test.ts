...
describe('getDbUserByEmail', () => {
  beforeEach<UserIdTestContext>((context: UserIdTestContext & TestContext) => {
    setupSessionContext(context);
    setRetrieveUserDataFunction(undefined!);
  });

  test<UserIdTestContext>('Should call retrieveUserData if function is set', async (context) => {
  ...
  test<UserIdTestContext>('Should return a default user object if retrieveUserData is not set', async (context) => {
