...
describe('handler.handleSessionCookie', () => {
  test('Should generate a new session ID if the current session ID given is invalid and set the new value as a cookie.', async () => {
    const sessionOptions = {
      name: 'test.sid',
    };
    const testTessionId = generateNewSessionId();
    // TODO: enable header session id checking.
    ...
    const response = await supertest(app)
      .get('/')
      .set(sessionOptions.name, testTessionId)
      .set('Content-Type', 'application/json');

    expect(response.status).toBe(HttpStatusCode.UNAUTHORIZED);
    // TODO: Set checkMultiple when we fix sessionId/connect.sid issue.
    expectResponseResetsSessionIdCookie(response, testTessionId, sessionOptions.name, false);
  });

  test('Should not match sessionId in a cookie string.', () => {
    expectDifferentSetCookieSessionId(
      'sessionId',
      'a2146a0b-579e-5483-ada6-9d6e1ccfe984',
      'sessionId=d9cac899-f89f-49c7-b945-6ec44e9314c7; Path=/; HttpOnly; SameSite=Strict'
    );
  });

  test('Should not match test.sid in a cookie string.', () => {
    expectDifferentSetCookieSessionId(
      'test.sid',
      'a2146a0b-579e-5483-ada6-9d6e1ccfe984',
      'test.sid=d9cac899-f89f-49c7-b945-6ec44e9314c7; Path=/; HttpOnly; SameSite=Strict'
    );
  });
  ...
  test('Should match sessionId in a cookie string.', () => {
    expectSetCookieSessionId(
      'sessionId',
      'd9cac899-f89f-49c7-b945-6ec44e9314c7',
      'sessionId=d9cac899-f89f-49c7-b945-6ec44e9314c7; Path=/; HttpOnly; SameSite=Strict'
    );
  });

  test('Should match test.sid in a cookie string.', () => {
    expectSetCookieSessionId(
      'test.sid',
      'd9cac899-f89f-49c7-b945-6ec44e9314c7',
      'test.sid=d9cac899-f89f-49c7-b945-6ec44e9314c7; Path=/; HttpOnly; SameSite=Strict'
