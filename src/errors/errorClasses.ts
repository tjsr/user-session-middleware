...
export class RequiredMiddlewareNotCalledError extends SessionHandlerError {
  constructor(requiredHandler: Handler | ErrorRequestHandler, currentHandler: Handler | ErrorRequestHandler) {
    super(
      ...
      `Prerequisite handler ${requiredHandler.name} not called before ${currentHandler.name}.`
    );
  }
}
...
export class ProductionModeConfigurationError extends MiddlewareConfigurationError {
  constructor(optionType: string) {
    super('USM Configuration option is not permitted in production mode: ' + optionType);
  }
}

export class ConfigurationOptionRequiredError extends MiddlewareConfigurationError {
  constructor(optionType: keyof UserSessionOptions) {
    super(`USM option ${optionType} is required`);
  }
}
...
export class SessionDataNotExpectedError extends SessionHandlerError {
  constructor() {
    super(NEW_SESSION_ID_DATA_EXISTS, HttpStatusCode.UNAUTHORIZED, 'Session data received for new session ID.');
  }
}

export class SessionIDNotGeneratedError extends SessionHandlerError {
  constructor() {
    super(ERROR_SESSION_ID_NOT_GENERATED, HttpStatusCode.NOT_IMPLEMENTED, 'Session ID not generated for request.');
  }
}

export class RegeneratingSessionIdError extends SessionHandlerError {
  constructor(cause: unknown) {
    super(SESSION_ID_GENERATION_ERROR, HttpStatusCode.INTERNAL_SERVER_ERROR, 'Error regenerating session ID.', cause);
  }
}
...
export class SessionUserInfoError extends SessionHandlerError {
  constructor(message: string, code: HttpStatusCode = HttpStatusCode.UNAUTHORIZED) {
    super(ERROR_SESSION_VALUES_MISSING, code, message);
  }
}

export abstract class RequestInvalidError extends SessionHandlerError {
  constructor(
    sesisonErrorCode: number,
    status?: number,
    message: string = 'Request contained invalid data',
    cause?: unknown
  ) {
    super(sesisonErrorCode, status, message, cause);
  }
}

export class RequestHeadersInvalidError extends RequestInvalidError {
  constructor(
    sesisonErrorCode: number,
    status?: number,
    message: string = 'Request contained invalid header data',
    cause?: unknown
  ) {
    super(sesisonErrorCode, status, message, cause);
  }
...
export class SetCookieHeaderNotPermittedError extends RequestHeadersInvalidError {
  constructor() {
    super(SET_COOKIE_NOT_PERMITTED, HttpStatusCode.BAD_REQUEST, 'Set-Cookie is not permitted in request headers');
  }
}
